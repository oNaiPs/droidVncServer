...
static void *httpListen(void *data)
{
    rfbScreenInfoPtr screen=(rfbScreenInfoPtr)data;
    while (1)
    {
    rfbHttpCheckFds(screen);//backgrounded event loop doesnt listen for connections from browser
    usleep(100000);
    }
    return NULL;
}
...
void rfbRunEventLoop(rfbScreenInfoPtr screen, long usec, rfbBool runInBackground)
{
  if(runInBackground) {
       ...
       pthread_create(&listener_thread, NULL, httpListen, screen);
    return;
#else
